...
2) POST any path under `/api/messages/` (for example, `/api/messages/123`)
   - Return HTTP 200 with an empty response body
   - This route should match any path under `/api/messages/`. In Convex, use `pathPrefix: "/api/messages/"`.
```
